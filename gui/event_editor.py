# -*- coding: utf-8 -*-
"""
MHXY GUI 自动化脚本平台 - 事件编辑器对话框（Task 12）。

本模块实现 ``EventEditorDialog(QDialog)``，提供对单个 ``Event`` 对象的
可视化编辑能力，覆盖以下内容：

    - 通用属性：事件名称、是否启用、执行前/后延迟、错误处理策略、最大重试次数
    - 事件参数：根据事件类型切换不同的编辑页面，支持全部 7 种事件类型：
        * click      —— 鼠标点击（坐标、按键、后台点击）
        * key        —— 键盘输入（按键组合、文本、持续时间）
        * wait       —— 等待延迟（时长、等待图像、超时）
        * image      —— 图像识别（模板路径、阈值、动作、识别区域）
        * yolo       —— YOLO 检测（目标类别、置信度、动作、模型路径）
        * function   —— 函数调用（模块、函数、args/kwargs JSON）
        * condition  —— 条件分支（变量、运算符、比较值；分支序列在外部编辑）

使用方式::

    from gui.event_editor import EventEditorDialog
    dialog = EventEditorDialog(event, parent)
    if dialog.exec_() == QDialog.Accepted:
        # EventEditorDialog.accept 已将修改写回 event 对象
        ...

设计要点：
    - 控件值不实时回写 Event，仅在点击"确定"时统一通过 ``_apply_to_event``
      写回；点击"取消"则保持原值。
    - 函数调用页的 args/kwargs 以 JSON 字符串输入，解析失败时弹出警告并阻止
      对话框关闭。
    - 函数调用页的模块下拉从 ``task_library.get_enabled_modules()`` 获取，
      模块切换时刷新函数下拉；task_library 缺失或为空时降级为空列表并给出
      提示，不导致崩溃。
"""
import json
import os
...
except Exception as e:  # pragma: no cover - 依赖缺失时的兜底
    task_library = None
    logger.warning(f"event_editor: 未能导入 task_library，函数页将降级 - {e}")


# ----------------------------------------------------------------------
# 事件类型中文名映射（避免与 task_editor 中的同名函数形成强耦合）
# ----------------------------------------------------------------------
_EVENT_TYPE_DISPLAY = {
    EventType.CLICK: "鼠标点击",
    EventType.KEY: "键盘输入",
    EventType.WAIT: "等待延迟",
    EventType.IMAGE: "图像识别",
    EventType.YOLO: "YOLO 检测",
    EventType.FUNCTION: "函数调用",
    EventType.CONDITION: "条件分支",
}


def _event_type_display_name(event_type):
    """返回事件类型对应的中文名，未知类型回退到类型字符串本身。"""
    return _EVENT_TYPE_DISPLAY.get(event_type, str(event_type))
...
class EventEditorDialog(QDialog):
    ...
    接收一个 ``Event`` 对象，在对话框中编辑其属性与参数；点击"确定"后
    修改会写回原 Event 对象（即原地修改，调用方无需重新装配）。

    :param event: 要编辑的 Event 实例
    :param parent: 父窗口
    :param previous_events: 前序事件列表（用于承接参数下拉选择）
    """

    def __init__(self, event: Event, parent=None, previous_events=None):
        super().__init__(parent)
        if event is None:
            # 防御性：调用方不应传入 None，但若发生则构造一个空事件避免崩溃
            logger.warning("EventEditorDialog 收到 None 事件，将使用空事件占位。")
            event = Event(name="", event_type=EventType.CLICK)

        self._event = event
        # 前序事件列表（用于承接参数）
        self._previous_events = previous_events or []
        ...
        logger.debug(
            f"EventEditorDialog 已加载事件: id={event.id[:8]}, "
            f"type={event.event_type}, name={event.name!r}"
        )
    ...
    def _init_ui(self):
        ...
        # 3. 参数编辑区
        layout.addWidget(self._build_params_group(), 1)
    ...
    def _build_common_group(self) -> QGroupBox:
        """构建"通用属性"分组：名称 / 启用 / 延迟 / 错误处理 / 重试次数。"""
        group = QGroupBox("通用属性")
        form = QFormLayout(group)
        form.setLabelAlignment(Qt.AlignRight)

        # 事件名称
        self._name_edit = QLineEdit()
        self._name_edit.setPlaceholderText("事件名称")
        form.addRow("事件名称：", self._name_edit)

        # 变量别名（用于后续事件引用结果）
        self._var_name_edit = QLineEdit()
        self._var_name_edit.setPlaceholderText("变量别名（留空自动用事件名，如 JHRW）")
        self._var_name_edit.setToolTip(
            "该事件结果的变量名，后续事件可通过 ${别名.字段} 引用"
        )
        form.addRow("变量别名：", self._var_name_edit)
        ...
        # 执行前延迟（秒）
        self._pre_delay_spin = QDoubleSpinBox()
        self._pre_delay_spin.setRange(0.0, 86400.0)
        self._pre_delay_spin.setSingleStep(0.1)
        self._pre_delay_spin.setDecimals(3)
        self._pre_delay_spin.setSuffix(" 秒")
        form.addRow("执行前延迟：", self._pre_delay_spin)

        # 执行后延迟（秒）
        self._post_delay_spin = QDoubleSpinBox()
        self._post_delay_spin.setRange(0.0, 86400.0)
        self._post_delay_spin.setSingleStep(0.1)
        self._post_delay_spin.setDecimals(3)
        self._post_delay_spin.setSuffix(" 秒")
        form.addRow("执行后延迟：", self._post_delay_spin)

        # 错误处理策略
        self._on_error_combo = QComboBox()
        self._on_error_combo.addItem("重试 (retry)", "retry")
        self._on_error_combo.addItem("跳过 (skip)", "skip")
        self._on_error_combo.addItem("停止 (stop)", "stop")
        form.addRow("错误处理策略：", self._on_error_combo)

        # 最大重试次数
        self._max_retries_spin = QSpinBox()
        self._max_retries_spin.setRange(0, 99)
        form.addRow("最大重试次数：", self._max_retries_spin)
    ...
    def _build_params_group(self) -> QGroupBox:
        """
        构建"事件参数"分组：使用 QStackedWidget，根据事件类型切换页面。

        通过 ``_page_index`` 字典维护 event_type -> stack 下标的映射，
        便于在加载事件时快速切换到对应页面。
        """
        group = QGroupBox("事件参数")
        ...
        # 依次创建各事件类型对应的参数页
        builders = [
            (EventType.CLICK, self._build_click_page),
            (EventType.KEY, self._build_key_page),
            (EventType.WAIT, self._build_wait_page),
            (EventType.IMAGE, self._build_image_page),
            (EventType.YOLO, self._build_yolo_page),
            (EventType.FUNCTION, self._build_function_page),
            (EventType.CONDITION, self._build_condition_page),
        ]
        for event_type, builder in builders:
            page = builder()
            idx = self._params_stack.addWidget(page)
            self._page_index[event_type] = idx
    ...
    # 各事件类型参数页
    # ------------------------------------------------------------------
    def _build_click_page(self) -> QWidget:
        ...
        # 2026-08-05 新增：按下→弹起延迟(ms)。后台模式下 DOWN 后保持该时长
        # 再发 UP。部分游戏要求按下保持 ≥30~50ms 才判定有效点击；偶发点击
        # 失效（角色没动）时可调大到 100~300ms。
        self._click_press_delay_spin = QSpinBox()
        self._click_press_delay_spin.setRange(0, 2000)
        self._click_press_delay_spin.setSingleStep(10)
        self._click_press_delay_spin.setSuffix(" ms")
        self._click_press_delay_spin.setValue(50)
        self._click_press_delay_spin.setToolTip(
            "按下→弹起之间的保持时间（毫秒）。点击偶发失效时调大到 100~300"
        )
        ...
        # 获取前序函数调用事件
        func_events = self._get_previous_function_events()
    ...
    # 前序事件辅助方法
    # ------------------------------------------------------------------
    def _get_previous_function_events(self):
        """获取前序事件中的函数调用事件列表（排除当前事件）。"""
        result = []
        for evt in self._previous_events:
            if evt.id != self._event.id and evt.event_type == EventType.FUNCTION:
                result.append(evt)
        return result
    ...
    def _fill_click_coord(self, target):
        ...
        if not template:
            QMessageBox.warning(self, "提示", "请先选择来源事件和字段")
            return
        if target == "x":
    ...
    def _fill_click_coord_xy(self):
        ...
        src = self._get_selected_source_event()
        if src is None:
            QMessageBox.warning(self, "提示", "请先选择来源事件")
            return
        var_name = src.var_name if src.var_name else src.name.replace(" ", "").replace("_", "")
    ...
    def _build_image_page(self) -> QWidget:
        ...
        # 来源事件
        prev_func_events = self._get_previous_function_events()
        row1 = QHBoxLayout()
        row1.addWidget(QLabel("来源事件："))
        ...
        dir_browse_btn.clicked.connect(
            lambda: self._on_browse_dir(self._image_dyn_dir_edit)
        )
        ...
        batch_dir_browse_btn.clicked.connect(
            lambda: self._on_browse_dir(self._image_batch_dir_edit)
        )
    ...
    def _on_browse_dir(self, line_edit: QLineEdit):
        """通用目录浏览辅助方法：弹出 QFileDialog 选择目录。"""
    ...
    def _build_yolo_page(self) -> QWidget:
    ...
    def _on_yolo_fill_template(self):
        ...
        if not hasattr(self, '_yolo_source_combo'):
            QMessageBox.warning(self, "提示", "无前置函数调用事件")
            return
        ...
        if not field:
            QMessageBox.warning(self, "提示", "请先选择字段")
            return
        var_name = src.var_name if src.var_name else src.name.replace(" ", "").replace("_", "")
    ...
    def _build_function_page(self) -> QWidget:
        """
        函数调用参数页：模块 / 函数 / args(JSON) / kwargs(JSON) + 承接参数。
    ...
    def _edit_subflow_dialog(self, subflow: List[Event]) -> bool:
        ...
        2026-08-04 统一：使用 gui/subflow_editor.SubFlowEditorDialog
        （含"复制选中事件 / 粘贴事件"及 Ctrl/Shift 多选），删除本文件内
        旧的重复 5 按钮版本（曾导致用户看不到复制功能）。
        """
        from gui.subflow_editor import SubFlowEditorDialog as _UnifiedSubFlow
        dlg = _UnifiedSubFlow(subflow, self, self._previous_events)
    ...
    def _on_add_case(self):
        ...
        btn.clicked.connect(self._on_edit_case_subflow)
    ...
    def _on_edit_case_subflow(self):
    ...
    def _on_preset_cases_from_file(self):
        ...
        if not entries:
            QMessageBox.information(self, "提示", "文件中未找到有效条目")
            return
    ...
    # 数据加载 / 写回
    # ==================================================================
    def _load_from_event(self, event: Event):
        """
        将 Event 的属性与参数加载到 UI 控件。

        :param event: 要加载的 Event 实例
        """
        # 顶部类型显示
        self._type_label.setText(
            f"{_event_type_display_name(event.event_type)}  "
            f"({event.event_type})"
        )

        # 通用属性
        self._name_edit.setText(event.name or "")
        self._var_name_edit.setText(event.var_name or "")
        self._enabled_check.setChecked(bool(event.enabled))
        self._pre_delay_spin.setValue(float(event.pre_delay or 0.0))
        self._post_delay_spin.setValue(float(event.post_delay or 0.0))
        self._set_combo_by_data(self._on_error_combo, event.on_error, "skip")
        self._max_retries_spin.setValue(int(event.max_retries or 0))

        # 切换到对应参数页
        idx = self._page_index.get(event.event_type)
        if idx is None:
            # 未知事件类型：切到第一页（CLICK）作为兜底
            idx = self._page_index.get(EventType.CLICK, 0)
        self._params_stack.setCurrentIndex(idx)

        # 按事件类型加载具体参数
        params = event.params or {}
        loader = {
            EventType.CLICK: self._load_click_params,
            EventType.KEY: self._load_key_params,
            EventType.WAIT: self._load_wait_params,
            EventType.IMAGE: self._load_image_params,
            EventType.YOLO: self._load_yolo_params,
            EventType.FUNCTION: self._load_function_params,
            EventType.CONDITION: self._load_condition_params,
        }.get(event.event_type)
        if loader is not None:
            loader(params)

    def _apply_to_event(self, event: Event) -> bool:
        """
        将 UI 控件的值写回 Event 对象。

        :param event: 要写入的 Event 实例
        :return: 是否写入成功（如 JSON 解析失败会返回 False 并弹出警告）
        """
        # 通用属性
        event.name = self._name_edit.text().strip()
        event.var_name = self._var_name_edit.text().strip()
        event.enabled = self._enabled_check.isChecked()
        event.pre_delay = self._pre_delay_spin.value()
        event.post_delay = self._post_delay_spin.value()
        event.on_error = self._on_error_combo.currentData() or "skip"
        event.max_retries = self._max_retries_spin.value()

        # 按事件类型写回参数
        applier = {
            EventType.CLICK: self._apply_click_params,
            EventType.KEY: self._apply_key_params,
            EventType.WAIT: self._apply_wait_params,
            EventType.IMAGE: self._apply_image_params,
            EventType.YOLO: self._apply_yolo_params,
            EventType.FUNCTION: self._apply_function_params,
            EventType.CONDITION: self._apply_condition_params,
        }.get(event.event_type)
        if applier is not None:
            if not applier(event):
                return False
        return True
    ...
    # 各事件类型参数加载
    # ------------------------------------------------------------------
    def _load_click_params(self, params: dict):
        ...
        # 按下→弹起延迟（ms），默认 50
        try:
            press_ms = int(float(params.get("press_delay", 0.05) or 0.05) * 1000)
    ...
    def _load_wait_params(self, params: dict):
        """加载等待延迟参数到控件。"""
        try:
    ...
    def _load_image_params(self, params: dict):
        """加载图像识别参数到控件。"""
        # 模板来源模式
        source_mode = params.get("source_mode", "direct")
        self._set_combo_by_data(self._image_source_mode_combo, source_mode, "direct")
        ...
        if dyn_field_data == "__custom__":
            self._set_combo_by_data(self._image_dyn_field_combo, "__custom__", "target_location")
            self._image_dyn_custom_field_edit.setText(
                str(params.get("dyn_custom_field", "") or "")
            )
        else:
            self._set_combo_by_data(self._image_dyn_field_combo, dyn_field_data, "target_location")
        ...
        self._set_combo_by_data(self._image_add_mode_combo, add_mode, "direct")
        # 直接输入坐标
    ...
    def _load_yolo_params(self, params: dict):
    ...
    def _load_function_params(self, params: dict):
        ...
        # 选中对应模块（会触发函数列表刷新）
        self._set_combo_by_data(self._func_module_combo, module_name, "")
        # 刷新函数下拉后选中对应函数
        self._refresh_function_combo(module_name)
        self._set_combo_by_data(self._func_function_combo, function_name, "")
        ...
        if validate_field and validate_whitelist:
            ...
            self._set_combo_by_data(self._func_validate_field_combo, validate_field, "target_location")
    ...
    def _load_condition_params(self, params: dict):
        """加载条件分支参数到控件（支持 simple 和 switch 两种模式）。"""
        mode = str(params.get("mode", "simple") or "simple").lower()
        self._set_combo_by_data(self._cond_mode_combo, mode, "simple")
        # 触发模式切换
    ...
    # 各事件类型参数写回
    # ------------------------------------------------------------------
    def _apply_click_params(self, event: Event) -> bool:
    ...
    def _apply_key_params(self, event: Event) -> bool:
    ...
    def _apply_wait_params(self, event: Event) -> bool:
        """将等待延迟参数写回 Event.params。"""
        event.params = {
    ...
    def _apply_image_params(self, event: Event) -> bool:
    ...
    def _apply_yolo_params(self, event: Event) -> bool:
    ...
    def _apply_function_params(self, event: Event) -> bool:
        """
    ...
    def _apply_condition_params(self, event: Event) -> bool:
        """
        ...
        if mode == "switch":
            ...
            if default_act == "subflow":
                ...
                if default_act == "click":
                    ...
                    default_action["coord_file"] = (
                        self._cond_default_file_edit.text().strip()
                    )
                    default_action["button"] = (
        ...
        parsed_value = self._try_parse_value(value_text)
        event.params = {
            "mode": "simple",
    ...
    # 通用辅助方法
    # ==================================================================
    def _on_browse_file(self, widget, filter: str, multi: bool = False):
        """
        通用文件浏览辅助方法：弹出 QFileDialog 选择文件，将路径填入 widget。

        :param widget: 用于显示所选路径的 QLineEdit 或 QTextEdit
        :param filter: 文件过滤器，如 "图片 (*.png *.jpg *.bmp *.jpeg)"
        :param multi: 是否支持多选文件（仅 QTextEdit 有效）
        """
        if multi and isinstance(widget, QTextEdit):
            paths, _ = QFileDialog.getOpenFileNames(
                self, "选择文件（可多选）", "", filter
            )
            if paths:
                existing = widget.toPlainText().strip()
                existing_paths = [
                    p for p in existing.splitlines() if p.strip()
                ] if existing else []
                all_paths = existing_paths + paths
                widget.setPlainText("\n".join(all_paths))
        else:
            path, _ = QFileDialog.getOpenFileName(
                self, "选择文件", "", filter
    ...
    @staticmethod
    def _set_combo_by_data(combo: QComboBox, data, default: str):
        """
        按 userData 选中 QComboBox 中对应项；未找到时回退到 default。

        :param combo: 目标 QComboBox
        :param data: 要匹配的 userData 值
        :param default: 未找到时回退的 userData
        """
        if data is None:
            data = default
    ...
    def _try_parse_value(text: str):
        """
        尝试用 json.loads 解析比较值，支持数字 / 布尔 / 字符串。

        - 空字符串：返回 ""
        - "true"/"false"（不区分大小写）：返回对应布尔值
        - 纯数字：返回 int/float
        - JSON 字符串（带引号）：返回去引号的字符串
        - 其它：返回原字符串
        """
        if not text:
            return ""
        # 优先尝试 JSON 解析（覆盖数字、布尔、带引号字符串）
        try:
            return json.loads(text)
        except (json.JSONDecodeError, ValueError):
            pass
        # 单独处理 true/false（用户可能直接输入 true 而非 "true"）
    ...
    # ==================================================================
    def accept(self):
        """重写 accept：将控件值写回 Event 后再关闭对话框。"""
        if not self._apply_to_event(self._event):
            # 写回失败（如 JSON 解析错误）：不关闭对话框，等待用户修正
            return
        logger.debug(
            f"EventEditorDialog 已写回事件: id={self._event.id[:8]}, "
            f"name={self._event.name!r}"
        )
        super().accept()


# ----------------------------------------------------------------------
# 模块自测：直接运行本文件时弹出事件编辑器，验证布局
# ----------------------------------------------------------------------
if __name__ == "__main__":
    ...
    # 自测：依次为每种事件类型弹一次对话框
    # 复用 task_editor 中的默认参数模板，保证加载时不会缺字段
    from gui.task_editor import _default_params  # noqa: WPS433

    for et in EventType.all():
        ev = Event(name=f"测试-{et}", event_type=et,
                   params=_default_params(et))
        dialog = EventEditorDialog(ev)
        if dialog.exec_() == QDialog.Accepted:
            logger.debug(f"[{et}] -> {ev.to_json()}")
        else:
            logger.debug(f"[{et}] -> 取消")

    sys.exit(0)


# ======================================================================
# 子流程编辑器：编辑某个 switch case（或 default）下的一组事件序列
# ======================================================================
