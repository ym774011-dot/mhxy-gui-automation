...
#   ★时长 8s（用户实测推定）：战斗中 _wait_task_done/自动kick 持续轮询 in_battle
#   会不断续期 → 战斗全程有效；战斗结束后 8s 即解闩——之前 20s 会把
#   顺手打稀有怪（战斗结束 ~10s 后扫描）和下一轮 CALL（~16s）一起拦死。
...
#   合成旗=杂货 天然隔离）+ 介绍含"装备角色"兜底。MHXY_ZG_SELL=0 可整体关闭。
#   ★2026-09-12/13 用户定案：魔兽要诀、上古锻造图策 移出出售判据
#   （改由存仓流程归档；魔兽要诀不可叠加，存仓豁免 99 攒堆规则）。
# ============================================================
...
# ★2026-09-17 用户定案：额外出售白名单 / 内丹保留名单的「单一真源」。
#   全地图刷怪（run_unlimited_hunt）与「满包先卖后存」（warehouse_store）共用同一份，
#   避免两处副本漂移。注意：**不改** zhuagui_sell_junk 的默认空参——抓鬼链路不受影响；
#   仅由显式传入 extra_sell/extra_exclude 的调用方启用。
#   白名单：百炼精铁/制造指南书/钨金/内丹（打造/功能/召唤兽材料，不在 武器/防具 基础判据内）。
#   保留名单：召唤兽内丹/高级召唤兽内丹 中含这些子串的**不卖**（其余内丹一律卖）。
SELL_EXTRA_JUNK = ("百炼精铁", "制造指南书", "钨金", "内丹")
SELL_EXTRA_KEEP = ("矫健", "迅敏", "圣洁", "狂怒", "玉砥柱",
                   "生死决", "双星爆", "灵身", "静岳")
...
# 身体点击：世界脚底锚点上移量（半身高；2026-09-07 用户标定 40 →
# 2026-09-08 晚用户实况再上移 10px → 50，队员点队长身体更稳）
_TEAM_BODY_LIFT = 40   # ★2026-09-10 用户定案：50 → 40（50 会点到头上方，40 正好命中身体）
...
"""
    ...
    _call_guard["gid"] = gid
    _call_guard["ts"] = _now
    # ★2026-09-07 用户要求：CALL 后至少 0.5s 再做任何点击——对话框弹出有
    #   渲染延迟，点太快会点在场景上（角色走动/取消对话），运行反而更慢。
    #   统一收在 CALL 发送点，所有调用方（首轮/轮询/重CALL）自动生效。
    _sleep(random.uniform(0.5, 0.9))
    return True
...
-- ★2026-09-18 修正（pid1844 实测）：状态 与 可视 无关 —— 脱战时 可视=false，
--   但 状态='取消'（已开）。旧版写 `vis and tostring(a.状态) or '-'`，状态被 vis
--   挡死 → 永远读成 '-'→None（这才是"状态判定看着不可用"的真因，不是字段不存在）。
local st = ''
if type(a) == 'table' then
  local ok, s = pcall(function() return a.状态 end)
  if ok and s ~= nil then st = tostring(s) end
end
__out = (inb and '1' or '0') .. '|' .. (st ~= '' and st or '-')
"""
...
# ★2026-09-18 **真机实测**（pid1844；用户提示"自动面板上有 回合/取消/重置/暂离"后挖到）：
#   状态字段**确实存在** —— `tp.战斗类.窗口.自动栏.状态`：**'取消' = 自动已开；'自动' = 未开**。
#   之前"本服没有该字段"是**误判**，真因在 `_AUTO_STATE_LUA`：状态 被挂在 `可视` 之后
#   （`vis and tostring(a.状态) or '-'`），而实测 `可视=false` 时 状态 依然有值
#   （脱战残留 = '取消'）→ 永远读成 None。
#   → 现在**直读 状态**；策略：'取消'→不点；'自动'→点；读不到 → 退回"本会话每 pid 点一次"闸兜底。
_AUTO_ONCE = {}              # pid -> True：状态读不到时的兜底闸（每会话只点一次）
_AUTO_LOCK = _threading.Lock()


def zhuagui_ensure_auto_battle(hwnd=None, gateway=DEFAULT_GATEWAY, log=None, **kw):
    """进战斗时确保「自动」开启（★2026-09-18：状态优先 + 会话闸兜底）。

    读 `自动栏.状态`：'取消'=已开→不点（防点关）；'自动'=未开→点一次。
    状态读不到（None/''）→ 退回 `_AUTO_ONCE`：本会话每 pid 只点一次。
...
''')
        if st == "取消":                 # 明确已开 → 不点（防把开关点关）
            return "auto_on"
        if st != "自动" and _AUTO_ONCE.get(pid):
            return "auto_on"             # 状态读不到 → 退回会话闸（本会话已点过就不再点）
        _AUTO_ONCE[pid] = True
        x0, y0, x1, y1 = _AUTO_BTN_RECT
