...
校准来源（终端 #792-829）:
  - 地图原点 (0,0) = 客户区像素 (220, 199)
  - 缩放比例: x=1.954 px/unit, y=1.955 px/unit
  - 验证: 像素(347,418) → 游戏坐标(65.0, 112.0) ✓
  - 像素(357,280) → 游戏坐标(70.1, 41.4) ✓
...
使用方式:
  1. 作为模块导入:
     from JYC import JYC
     result = JYC((65, 112))                      # 点击游戏坐标(65,112)
     result = JYC((65, 112), pid=28024)           # 指定PID
     result = JYC((65, 112), click=False)         # 只算坐标不点击
     result = JYC((65, 112), background=False)    # 前台点击（移动鼠标）

  2. 命令行:
     python JYC.py 65,112                         # 点击
     python JYC.py 65,112 -p 28024                # 指定PID
     python JYC.py 65,112 -n                      # 只算坐标不点击
     python JYC.py 65,112 -f                      # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-07-30 实测，终端 #792-829）
# ============================================================
MAP_ORIGIN_PIXEL = (220, 199)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (1.954, 1.955)         # 每游戏单位对应像素数 (x, y)
# 大地图有效点击范围上限（用户实测，与 data/map_ui_blocks.json max_game_coord 一致；None=暂无数据不限制）
MAP_MAX_GAME_COORD = (284, 140)
...
# 主函数 JYC
# ============================================================
def JYC(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    建邺城 地图坐标点击函数

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (65, 112)
...
def main():
    if len(sys.argv) < 2:
        print("JYC - 地图坐标点击函数")
        print("用法:")
        print("  python JYC.py 65,112                  # 后台点击游戏坐标(65,112)")
        print("  python JYC.py 65,112 -p 28024         # 指定PID")
        print("  python JYC.py 65,112 -n               # 只算坐标不点击")
        print("  python JYC.py 65,112 -f               # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
    ...
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 65,112")
        return
    try:
    ...
    result = JYC((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
