...
校准来源（终端 #561-599）:
  - 地图原点 (0,0) = 客户区像素 (359, 199)
  - 缩放比例: x=2.361 px/unit, y=2.342 px/unit
  - 验证: 像素(444,459) → 游戏坐标(36.0, 111.0) ✓
  - 像素(585,348) → 游戏坐标(95.7, 63.6) ✓
...
使用方式:
  1. 作为模块导入:
     from DHW import DHW
     result = DHW((36, 111))                     # 点击游戏坐标(36,111)
     result = DHW((36, 111), pid=28024)          # 指定PID
     result = DHW((36, 111), click=False)        # 只算坐标不点击
     result = DHW((36, 111), background=False)   # 前台点击（移动鼠标）

  2. 命令行:
     python DHW.py 36,111                        # 点击
     python DHW.py 36,111 -p 28024               # 指定PID
     python DHW.py 36,111 -n                     # 只算坐标不点击
     python DHW.py 36,111 -f                     # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-07-30 实测，终端 #561-599）
# ============================================================
MAP_ORIGIN_PIXEL = (359, 199)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (2.361, 2.342)         # 每游戏单位对应像素数 (x, y)
# 大地图有效点击范围上限（用户实测，与 data/map_ui_blocks.json max_game_coord 一致；None=暂无数据不限制）
MAP_MAX_GAME_COORD = (117, 117)
    ...
    for pid in candidates:
        ...
        if hwnd and any(kw in (title or '') for kw in _GAME_TITLE_KEYWORDS):
            if preferred_pid and pid != preferred_pid and verbose:
                print(f"[DHW] 兜底: 用枚举到的 PID={pid} 定位窗口 (原 PID={preferred_pid} 失效)")
            return hwnd, title
    return None, None
...
# 主函数 DHW
# ============================================================
def DHW(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    东海湾 地图坐标点击函数

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (36, 111)
    ...
    """
    # 兼容两种调用形式:
    #   DHW((36, 111))   -> target_coord 是二元组（标准写法）
    #   DHW(36, 111)     -> 误把 gx 当 target_coord、gy 当 pid 传进来
...
def main():
    if len(sys.argv) < 2:
        print("DHW - 东海湾地图坐标点击函数")
        print("用法:")
        print("  python DHW.py 36,111                # 后台点击游戏坐标(36,111)")
        print("  python DHW.py 36,111 -p 28024       # 指定PID")
        print("  python DHW.py 36,111 -n             # 只算坐标不点击")
        print("  python DHW.py 36,111 -f             # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
    ...
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 36,111")
        return
    try:
    ...
    result = DHW((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
