...
校准数据已内置（基于 2026-08-27 用户重校准），无需重新校准

校准来源:
  - 地图原点 (0,0) = 客户区像素 (278, 172)
  - 缩放比例: x=2.786 px/unit, y=2.792 px/unit
  - 验证: 像素(604,306) → 游戏坐标(117.0, 48.0) ✓
...
使用方式:
  1. 作为模块导入:
     from BXG import BXG
     result = BXG((117, 48))                     # 点击游戏坐标(117,48)
     result = BXG((117, 48), pid=28024)          # 指定PID
     result = BXG((117, 48), click=False)        # 只算坐标不点击
     result = BXG((117, 48), background=False)   # 前台点击（移动鼠标）

  2. 命令行:
     python BXG.py 117,48                        # 点击
     python BXG.py 117,48 -p 28024               # 指定PID
     python BXG.py 117,48 -n                     # 只算坐标不点击
     python BXG.py 117,48 -f                     # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-08-27 用户重校准）
#   角色@像素(604,306) → 游戏坐标(117,48)
#   反推原点: (604-117*2.786, 306-48*2.792) ≈ (278, 172)
#   验证: 像素(604,306) → 游戏坐标(117.0, 48.0) ✓
# ============================================================
MAP_ORIGIN_PIXEL = (278, 172)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (2.786, 2.792)         # 每游戏单位对应像素数 (x, y)
...
# 主函数 BXG
# ============================================================
def BXG(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    宝象国 地图坐标点击函数

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (117, 48)
...
def main():
    if len(sys.argv) < 2:
        print("BXG - 地图坐标点击函数")
        print("用法:")
        print("  python BXG.py 117,48                 # 后台点击游戏坐标(117,48)")
        print("  python BXG.py 117,48 -p 28024        # 指定PID")
        print("  python BXG.py 117,48 -n              # 只算坐标不点击")
        print("  python BXG.py 117,48 -f              # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
    ...
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 117,48")
        return
    try:
    ...
    result = BXG((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
