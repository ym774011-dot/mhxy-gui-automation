...
校准来源（终端 #654-691）:
  - 地图原点 (0,0) = 客户区像素 (314, 200)
  - 缩放比例: x=2.287 px/unit, y=2.276 px/unit
  - 验证: 像素(561,423) → 游戏坐标(108.0, 98.0) ✓
  - 像素(500,324) → 游戏坐标(81.3, 54.5) ✓

依赖: 无（纯 ctypes + Win32 API）

使用方式:
  1. 作为模块导入:
     from XLNR import XLNR
     result = XLNR((108, 98))                    # 点击游戏坐标(108,98)
     result = XLNR((108, 98), pid=28024)         # 指定PID
     result = XLNR((108, 98), click=False)       # 只算坐标不点击
     result = XLNR((108, 98), background=False)  # 前台点击（移动鼠标）

  2. 命令行:
     python XLNR.py 108,98                       # 点击
     python XLNR.py 108,98 -p 28024              # 指定PID
     python XLNR.py 108,98 -n                    # 只算坐标不点击
     python XLNR.py 108,98 -f                    # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-07-30 实测，终端 #654-691）
# ============================================================
MAP_ORIGIN_PIXEL = (314, 200)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (2.287, 2.276)         # 每游戏单位对应像素数 (x, y)
...
# 主函数 XLNR
# ============================================================
def XLNR(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    西凉女国 地图坐标点击函数

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (108, 98)
...
def main():
    if len(sys.argv) < 2:
        print("XLNR - 地图坐标点击函数")
        print("用法:")
        print("  python XLNR.py 108,98                # 后台点击游戏坐标(108,98)")
        print("  python XLNR.py 108,98 -p 28024       # 指定PID")
        print("  python XLNR.py 108,98 -n             # 只算坐标不点击")
        print("  python XLNR.py 108,98 -f             # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
    ...
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 108,98")
        return
    try:
    ...
    result = XLNR((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
