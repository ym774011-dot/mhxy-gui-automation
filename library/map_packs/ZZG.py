...
校准来源（终端 #746-783）:
  - 地图原点 (0,0) = 客户区像素 (278, 200)
  - 缩放比例: x=2.320 px/unit, y=2.333 px/unit
  - 验证: 像素(575,284) → 游戏坐标(128.0, 36.0) ✓
  - 像素(520,366) → 游戏坐标(104.3, 71.1) ✓
...
使用方式:
  1. 作为模块导入:
     from ZZG import ZZG
     result = ZZG((128, 36))                     # 点击游戏坐标(128,36)
     result = ZZG((128, 36), pid=28024)          # 指定PID
     result = ZZG((128, 36), click=False)        # 只算坐标不点击
     result = ZZG((128, 36), background=False)   # 前台点击（移动鼠标）

  2. 命令行:
     python ZZG.py 128,36                        # 点击
     python ZZG.py 128,36 -p 28024               # 指定PID
     python ZZG.py 128,36 -n                     # 只算坐标不点击
     python ZZG.py 128,36 -f                     # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-07-30 实测，终端 #746-783）
# ============================================================
MAP_ORIGIN_PIXEL = (278, 200)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (2.320, 2.333)         # 每游戏单位对应像素数 (x, y)
...
# 主函数 ZZG
# ============================================================
def ZZG(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    朱紫国 地图坐标点击函数

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (128, 36)
...
def main():
    if len(sys.argv) < 2:
        print("ZZG - 地图坐标点击函数")
        print("用法:")
        print("  python ZZG.py 128,36                 # 后台点击游戏坐标(128,36)")
        print("  python ZZG.py 128,36 -p 28024        # 指定PID")
        print("  python ZZG.py 128,36 -n              # 只算坐标不点击")
        print("  python ZZG.py 128,36 -f              # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
    ...
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 128,36")
        return
    try:
    ...
    result = ZZG((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
