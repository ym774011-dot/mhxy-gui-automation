# -*- coding: utf-8 -*-
"""
CAC -  长安城 地图坐标点击函数（1:1 像素映射）
================================
功能: 输入游戏逻辑坐标 → 自动计算地图像素位置 → 后台点击
校准数据已内置（基于 2026-07-30 实测校准），无需重新校准

校准来源（终端 #509-553）:
  - 地图原点 (0,0) = 客户区像素 (225, 200)
  - 缩放比例: x=1.000 px/unit, y=1.000 px/unit
  - 验证: 像素(583,235) → 游戏坐标(358.0, 35.0) ✓
  - 像素(540,320) → 游戏坐标(315.0, 120.0) ✓
  - 像素(644,258) → 游戏坐标(419.0, 58.0) ✓
...
使用方式:
  1. 作为模块导入:
     from CAC import CAC
     result = CAC((358, 35))                     # 点击游戏坐标(358,35)
     result = CAC((358, 35), pid=28024)          # 指定PID
     result = CAC((358, 35), click=False)        # 只算坐标不点击
     result = CAC((358, 35), background=False)   # 前台点击（移动鼠标）

  2. 命令行:
     python CAC.py 358,35                        # 点击
     python CAC.py 358,35 -p 28024               # 指定PID
     python CAC.py 358,35 -n                     # 只算坐标不点击
     python CAC.py 358,35 -f                     # 前台点击（默认后台）
"""
# ============================================================
# 函数中文元信息（GUI 下拉框显示用）
...
# ============================================================
# 内置校准数据（2026-07-30 实测，终端 #509-553）
# ============================================================
MAP_ORIGIN_PIXEL = (225, 200)      # 地图(0,0)在客户区的像素坐标
MAP_SCALE = (1.000, 1.000)         # 每游戏单位对应像素数 (x, y) — 1:1 映射
...
# 主函数 CAC
# ============================================================
def CAC(target_coord, *more, pid=DEFAULT_PID, click=True, background=False, verbose=False):
    """
    长安城 地图坐标点击函数（1:1 像素映射）

    参数:
        target_coord : (gx, gy) 目标游戏坐标，例如 (358, 35)
...
def main():
    if len(sys.argv) < 2:
        print("CAC - 地图坐标点击函数（1:1 像素映射）")
        print("用法:")
        print("  python CAC.py 358,35               # 后台点击游戏坐标(358,35)")
        print("  python CAC.py 358,35 -p 28024      # 指定PID")
        print("  python CAC.py 358,35 -n            # 只算坐标不点击")
        print("  python CAC.py 358,35 -f            # 前台点击(移动鼠标)")
        print()
        print("内置校准:")
        print(f"  地图原点(0,0) = 客户区像素 {MAP_ORIGIN_PIXEL}")
        print(f"  缩放比例 = {MAP_SCALE} px/unit (1:1 映射)")
        return

    # 解析坐标
    coord_str = sys.argv[1]
    if ',' not in coord_str:
        print("坐标格式错误，需要 x,y 格式，例如 358,35")
        return
    try:
    ...
    result = CAC((gx, gy), pid=pid, click=click, background=background, verbose=verbose)
    if not result['ok']:
        print(result['message'])
